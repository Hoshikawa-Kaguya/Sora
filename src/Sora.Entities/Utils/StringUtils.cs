...
public static class StringUtils
{
    ...
    public static int ToIntOrDefault(this string str, int defaultValue = 0) =>
        int.TryParse(str, out int val) ? val : defaultValue;

    /// <summary>Parses the string as a <see cref="long" />, returning <paramref name="defaultValue" /> on failure.</summary>
    public static long ToLongOrDefault(this string str, int defaultValue = 0) =>
        long.TryParse(str, out long val) ? val : defaultValue;
}