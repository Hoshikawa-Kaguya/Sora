...
| NuGet 包 | 说明 |
|------|------|
| `HoshikawaKaguya.Sora` | 框架门面层 — SoraService、SoraServiceFactory、日志初始化 |
| `HoshikawaKaguya.Sora.Entities` | 共享实体 — 事件、消息段、信息类型、API 接口 |
| `HoshikawaKaguya.Sora.Core` | 核心工具 — 枚举、扩展方法、通用基础设施 |
| `HoshikawaKaguya.Sora.Command` | 属性指令路由 — `[CommandGroup]` + `[Command]` 声明式指令 |
...
> 
> 这个框架基于 [LuckyLilliaBot](https://github.com/LLOneBot/LuckyLilliaBot) 完成开发调试，目前测试只使用了 [LuckyLilliaBot](https://github.com/LLOneBot/LuckyLilliaBot) 进行测试
